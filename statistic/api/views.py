...
from statistic.api.serializers import GetGeneralStatisticsSerializer
...
def general_statistics(request):
    ...
    event = user.event_set.all().first()
    invitations = event.invitation_set.all()
    pending = invitations.filter(status="pending")
    waiting = invitations.filter(status="waiting")
    confirmed = invitations.filter(status="confirmed")
    apologized = invitations.filter(status="apologized")
    checked = invitations.filter(status="checked")
    statistics = {
    ...
    statistics_serializer = GetGeneralStatisticsSerializer(statistics)
